...
VOID ACTION_DESTORYWINDOW(HWND hWnd);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCEW(IDI_GDILEARN));
...
VOID ACTION_DESTORYWINDOW(HWND hWnd)
{
    if (MessageBox(hWnd, L"LET'S FUCK!", L"GameLearn", MB_OK) == IDOK)
    {
        DestroyWindow(hWnd);
    }
}
